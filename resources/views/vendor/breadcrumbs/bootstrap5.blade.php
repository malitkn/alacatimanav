    ...
    <section class="breadcrumb__section section--padding">
        <div class="container">
            <div class="breadcrumb__content text-center">
                <h1 class="breadcrumb__title h2"> {{ $breadcrumbs->last()->title }}</h1>
                <ul class="breadcrumb__menu d-flex justify-content-center">
                    @foreach ($breadcrumbs as $breadcrumb)

                        @if ($breadcrumb->url && !$loop->last)
                            <li class="breadcrumb__menu--items"><a class="breadcrumb__menu--link" href="{{ $breadcrumb->url }}">{{ $breadcrumb->title }}</a></li>
                        ...
                        @else
                            <li><span class="breadcrumb__menu--text">{{ $breadcrumb->title }}</span></li>
                        @endif
